...
not only is :py:func:`eval` unsafe, but that it is also a difficult
prospect to make any program that takes user input perfectly safe.  In
particular, if user input can cause Python to crash with a segmentation
fault, safety of your system cannot be guaranteed.  Asteval explicitly
forbids the exploits described in the above link, and works hard to prevent
malicious code from crashing Python or accessing the underlying operating
system.  That said, we cannot guarantee that asteval is completely safe
from malicious code.  We claim only that it is safer than the builtin
...
but it does eliminate classes of attacks known to seg-fault the Python
